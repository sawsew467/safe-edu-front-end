...
const DateTimeInput = React.forwardRef<HTMLInputElement, DateTimeInputProps>(
  (options: DateTimeInputProps, ref) => {
    ...
    const inputValue = useMemo(() => {
      ...
      if (isValid(date) && year > 1900 && year < currentYear) {
        return date;
      }
    ...
    const onSegmentNumberValueChange = useEventCallback(
      (num: string) => {
        ...
        if (segment.type !== "period") {
          ...
          const rawValue =
            segment.value.length < length
              ? parseInt(segment.value).toString()
              : parseInt(segment.value.substring(1)).toString();
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex h-10 items-center px-2 justify-start rounded-md border border-input bg-transparent text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground  disabled:cursor-not-allowed disabled:opacity-50",
          ...
          {inputValue ? (
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>
                  <p>
                    Vui lòng nhập giá trị hợp lệ. Đầu vào không được để trống và
                    phải nằm trong phạm vi từ năm 1900 đến {currentYear}.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
